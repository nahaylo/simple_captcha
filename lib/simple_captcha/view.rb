  ...
  module ViewHelper #:nodoc
    ...
    private
      ...
      def simple_captcha_field(options={})
        if options[:object]
          text_field(options[:object], :captcha, { :value => '', :autocomplete => 'off'}.merge(options[:field_options] || {})) +
          hidden_field(options[:object], :captcha_key, {:value => options[:field_value]})
        else
          text_field_tag(:captcha, nil, { :autocomplete => 'off' }.merge(options[:field_options] || {}))
        end
      end
